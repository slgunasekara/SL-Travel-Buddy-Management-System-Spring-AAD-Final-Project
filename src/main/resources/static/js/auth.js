...
const Auth = (() => {
  function authenticate(username, password) {
    return AuthApi.login(username, password).then(res => {
      const b = res.body;
      if (b.otpRequired) return { ok: true, otpRequired: true, userId: b.userId, username: b.username };
      const user = {
  ...
  function verifyTwoFactor(username, code) {
    return AuthApi.verifyTwoFactor(username, code).then(res => {
      const b = res.body;
      const user = {
        userId: b.userId, username: b.username, name: b.name, role: b.role,
        contact: b.contact, nic: b.nic, email: b.email, createdAt: b.createdAt,
      ...
      return { ok: true, user, token: b.token };
    }).catch(err => ({ ok: false, message: apiErrorMessage(err, "Invalid or expired code.") }));
  }
  ...
  return { authenticate, verifyTwoFactor, requestOtp, verifyOtp, markOtpUsed, resetPassword };
})();
