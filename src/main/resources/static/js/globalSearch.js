const GlobalSearch = (() => {
    ...
    function buildIndex(term) {
        ...
        tripsCache.forEach(tr => {
            if (`${tr.startLocation} ${tr.endLocation} ${tr.tripCategory}`.toLowerCase().includes(t)) {
                out.push({
                    group: "Trips",
                    icon: "route",
                    title: `${tr.startLocation} → ${tr.endLocation}`,
                    sub: `${tr.tripCategory} · ${Fmt.date(tr.tripDate)}`,
                    hash: "#/trips",
                    table: "trips",
                    id: tr.tripId
                });
        ...
        employeesCache.forEach(e => {
            if (`${e.empName} ${e.empCategory} ${e.empCategory2 || ""} ${e.contactNo}`.toLowerCase().includes(t)) {
                out.push({
                    ...
                    icon: "users",
                    title: e.empName,
                    sub: `${e.empCategory}${e.empCategory2 ? " / " + e.empCategory2 : ""} · ${e.contactNo}`,
                    hash: "#/employees",
                    table: "employees",
                    id: e.empId
                });
        ...
        eventsCache.forEach(ev => {
            if (`${ev.customerName} ${ev.startLocation} ${ev.endLocation}`.toLowerCase().includes(t)) {
                out.push({
                    ...
                    sub: `${ev.startLocation} → ${ev.endLocation}`,
                    hash: "#/events",
                    table: "events",
                    id: ev.eventId
                });
    ...
    function setJumpTarget(table, id) {
        pendingJump = {table, id};
    }
