    ...
    renderCrudPage(container, {
        ...
        columns: [
            ...
            {key: "name", label: "Name"},
            {
                key: "role",
                label: "Role",
                render: r => `<span class="badge badge--${r.role === "Owner" ? "amber" : "blue"}">${r.role}</span>`
            },
        ...
        async onUpdate(row) {
            if (editingOriginalRole && editingOriginalRole !== row.role) {
                try {
                    await apiRequest("PATCH", `/v1/user/${row.userId}/role`, {role: row.role});
                } catch (e) {
                    Toast.warning("Saved, but the role change failed: " + (e.message || "please retry."));
                }
            }
        },
        ...
        canDelete(id) {
            if (id === Session.currentUser().userId) return {
                blocked: true,
                reason: "You cannot delete your own account while logged in."
            };
