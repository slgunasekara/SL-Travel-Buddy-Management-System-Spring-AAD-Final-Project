async function renderPricesPage(container) {
    ...
    let pricesCache = [];
    try {
        const res = await apiRequest("GET", "/v1/price/all");
        pricesCache = res.body || [];
    } catch (err) {
        pricesCache = [];
    }

    function latestPrice(type) {
        const rows = pricesCache
    ...
    renderCrudPage(container, {
        ...
        columns: [
            {key: "updatePricesId", label: "ID"},
            {
                key: "updateType",
                label: "Type",
                render: r => `<span class="badge badge--${r.updateType === "FUEL" ? "amber" : "blue"}">${Fmt.escapeHtml(r.updateType)}</span>`
            },
            {
                key: "changeType",
                label: "Change",
                render: r => `<span class="badge badge--${r.changeType === "INCREMENT" ? "red" : "green"}">${r.changeType === "INCREMENT" ? "▲" : "▼"} ${Fmt.escapeHtml(r.changeType)}</span>`
            },
        ...
        onPrint(row) {
            PrintReceipt.priceUpdateReceipt(row);
        }
    });
}
