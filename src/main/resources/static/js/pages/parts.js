async function renderPartsPage(container) {
    ...
    try {
        const [bRes, mRes] = await Promise.all([
            ...
            apiRequest("GET", "/v1/maintenance/all")
        ]);
        busesCache = bRes.body || [];
        maintenanceCache = mRes.body || [];
    } catch (err) {
        Toast.error(apiErrorMessage(err, "Failed to load buses/maintenance records."));
    }
    ...
    const maintOptionsForBus = (busId) => maintenanceCache
        .filter(m => !busId || Number(m.busId) === Number(busId))
        .map(m => ({
            value: m.maintId,
            label: `#${m.maintId} — ${(m.maintenanceType || "").replace(/_/g, " ")} (${Fmt.date(m.serviceDate)})`
        }));
    ...
    renderCrudPage(container, {
        ...
        fields: [
            {
                ...
                required: true,
                options: busOptions(),
                onChange: (data) => refreshMaintOptions(data.busId)
            },
            {
                name: "maintId",
                label: "Linked Maintenance (optional)",
                type: "select",
                options: maintOptionsForBus(null)
            },
        ...
        columns: [
            ...
            {
                key: "photoFileName", label: "Bill Photo", sortable: false,
        ...
        searchKeys: [r => busNumber(r.busId), "partName", "supplierName"],
        ...
        onSelectRow(row) {
            refreshMaintOptions(row.busId);
        },
        onClearForm() {
            refreshMaintOptions(null);
        },
        onPrint(row) {
            PrintReceipt.partPurchaseReceipt(row, busNumber(row.busId));
        }
    });
