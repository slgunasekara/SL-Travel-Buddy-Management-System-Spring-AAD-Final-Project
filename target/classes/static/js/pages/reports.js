function renderReportsPage(container) {
    ...
    async function getReport(path, extraParams) {
        const params = new URLSearchParams({from: fromDate, to: toDate, ...(extraParams || {})});
        const res = await apiRequest("GET", `/v1/report/${path}?${params.toString()}`);
        return res.body;
    }

    async function summaryStats() {
        const s = await getReport("summary-stats");

        return {...s, totalSalary: s.salaries};
    }
    ...
    async function summaryCardsHtml() {
    ...
    async function renderTab() {
        ...
        <div class="grid-2">
          ...
          <div class="card">
            ...
            const rows = (await getReport("daily-profit")).sort((a, b) => a.date.localeCompare(b.date));
            ...
            renderDonutChart(qs("#overviewDonut"), {data: await getReport("expense-breakdown")});
            return;
        }

        if (activeTab === "income") {
            const rows = await getReport("income-report");
        ...
        <div class="card">
          ...
          <div class="table-wrap"><table class="data-table">
            ...
            qs("#printBtn")?.addEventListener("click", () => PrintReceipt.tablePrint({
                ...
                totalValue: Fmt.money(rows.reduce((a, r) => a + r.amount, 0))
            }));
            return;
        }

        if (activeTab === "salary") {
            const rows = await getReport("salary-report");
            host.innerHTML = `
        ${await summaryCardsHtml()}
        <div class="card">
        ...
        if (activeTab === "trip") {
            const rows = await getReport("trip-report");
            host.innerHTML = `
        ${await summaryCardsHtml()}
        <div class="card">
          ...
          <div class="table-wrap"><table class="data-table">
            ...
            <tbody>${rows.length ? rows.map(r => `<tr><td>#${r.tripId}</td><td>${Fmt.date(r.tripDate)}</td><td>${Fmt.escapeHtml(r.busNumber)}</td><td>${Fmt.escapeHtml(r.route)}</td><td><span class="badge badge--blue">${r.category}</span></td></tr>`).join("") : `<tr><td colspan="5"><div class="table-empty">No trips in this range.</div></td></tr>`}</tbody>
          </table></div>
        </div>`;
        ...
        if (activeTab === "daily") {
            const rows = await getReport("daily-profit");
            host.innerHTML = `
        ${await summaryCardsHtml()}
        <div class="card">
          ...
          <div class="table-wrap"><table class="data-table">
            ...
            qs("#printBtn")?.addEventListener("click", () => PrintReceipt.tablePrint({
                docTitle: "Daily Profit Breakdown",
                heading: "Daily Profit Breakdown",
                subheading: `${Fmt.date(fromDate)} – ${Fmt.date(toDate)}`,
                ...
                totalLabel: "Total Net Profit",
                totalValue: Fmt.money(rows.reduce((a, r) => a + r.netProfit, 0))
            }));
            return;
        }
        ...
        <div class="card filter-bar">
          <div class="form-field"><label>Year</label>
            <select id="yearSelect">${await yearOptions(year)}</select>
          </div>
        </div>
        <div id="monthlyHost"></div>`;
            const renderMonthly = async () => {
                const y = Number(qs("#yearSelect").value);
                const rows = await apiRequest("GET", `/v1/report/monthly-profit?year=${y}`).then(r => r.body);
        ...
        if (activeTab === "leaderboard") {
            const [routes, drivers, conductors] = await Promise.all([
                getReport("top-routes", {limit: 5}),
                getReport("top-drivers", {limit: 5}),
                getReport("top-conductors", {limit: 5})
            ]);
            host.innerHTML = `
        ${await summaryCardsHtml()}
        <div class="grid-2">
    ...
    async function yearOptions(selected) {
        let trips = [];
        try {
            trips = (await apiRequest("GET", "/v1/trip/all")).body || [];
        } catch (err) { /* fall through with an empty list — the selected/current year still shows */
        }
