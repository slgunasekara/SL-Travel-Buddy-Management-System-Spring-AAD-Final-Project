...
function fieldHtml(f, value) {
    ...
    if (f.type === "photo") {
        return PhotoUpload.renderField(f.name, val);
    }
...
function readFieldValue(f, container) {
    ...
    if (f.type === "photo") {
        return PhotoUpload.getValue(container || document, f.name) || "";
    }
...
async function renderCrudPage(container, cfg) {
    ...
    async function fetchRows() {
        try {
            const res = await apiRequest("GET", cfg.api.base + "/all");
            localRows = res.body || [];
        } catch (err) {
            Toast.error(apiErrorMessage(err, `Failed to load ${cfg.title.toLowerCase()}.`));
            localRows = [];
        }
    }
    ...
    const photoSetters = {}; // fieldName -> setValue(fileName), returned by PhotoUpload.wire()

    cfg.fields.forEach(f => {
        if (f.onChange && f.type !== "photo") {
        ...
        if (f.type === "photo") photoSetters[f.name] = PhotoUpload.wire(container, f.name, () => {
            if (f.onChange) f.onChange(readAllFields());
        });
    });
    ...
    function readAllFields() {
        const data = {};
        cfg.fields.forEach(f => {
            data[f.name] = readFieldValue(f, container);
        });
        return data;
    }
    ...
    function fillForm(row) {
        cfg.fields.forEach(f => {
            if (f.type === "photo") {
                if (photoSetters[f.name]) photoSetters[f.name](row[f.name] || null);
                return;
            }
            const el = qs("#f_" + f.name, container);
            if (!el) return;
    ...
    function clearForm() {
        ...
        cfg.fields.forEach(f => {
            if (f.type === "photo") {
                if (photoSetters[f.name]) photoSetters[f.name](null);
                return;
            }
            const el = qs("#f_" + f.name, container);
            if (!el) return;
    ...
    function renderTable() {
        ...
        qsa(".photo-thumb-cell[data-photo]", container).forEach(cell => {
            PhotoUpload.renderTableThumbnail(cell, cell.dataset.photo);
        });
        ...
        qsa("#dataTable tbody tr[data-id]", container).forEach(tr => {
            ...
            if (printBtn) printBtn.addEventListener("click", () => {
                const row = getRows().find(r => r[cfg.idField] === id);
    ...
    async function doSave() {
        ...
        if (cfg.onCreate) cfg.onCreate(data); // fields like createdBy/createdAt — id is NOT set here anymore, the backend generates it


        try {
            await apiRequest("POST", cfg.api.base, data);
            ...
            Toast.error(apiErrorMessage(err, `Failed to save this ${(cfg.singular || cfg.title).toLowerCase()}.`));
        }
    }
    ...
    async function doUpdate() {
        ...
        const payload = {[cfg.idField]: editingId, ...data};
        if (cfg.onUpdate) cfg.onUpdate(payload);

        try {
            await apiRequest("PUT", cfg.api.base + "/" + editingId, payload);
            ...
            Toast.error(apiErrorMessage(err, `Failed to update this ${(cfg.singular || cfg.title).toLowerCase()}.`));
        }
    }
    ...
    async function doDelete(id) {
        if (cfg.canDelete) {
            ...
            if (check && check.blocked) {
                Toast.error(check.reason || "Cannot delete this record — it is referenced elsewhere.");
                return;
            }
        }
        const ok = await confirmDialog({
            title: `Delete ${cfg.singular || cfg.title}`,
            message: `Are you sure you want to delete this ${(cfg.singular || cfg.title).toLowerCase()}?`,
            okText: "Delete",
            danger: true
        });
        if (!ok) return;
        ...
        try {
            await apiRequest("DELETE", cfg.api.base + "/" + id);
            ...
            Toast.error(apiErrorMessage(err, `Failed to delete this ${(cfg.singular || cfg.title).toLowerCase()}.`));
        }
    }
    ...
    function exportCsv() {
        ...
        a.href = url;
        a.download = `${cfg.table}_export_${today()}.csv`;
        a.click();
    ...
    async function importCsv(file) {
        ...
        if (rows.length < 2) {
            Toast.warning("CSV file looks empty.");
            return;
        }
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (cfg.onCreate) cfg.onCreate(data);
            try {
                await apiRequest("POST", cfg.api.base, data);
                imported++;
            } catch (err) {
        ...
        renderTable();
        if (cfg.onChange) cfg.onChange();
        if (imported > 0) Toast.success(`Imported ${imported} record${imported === 1 ? "" : "s"}.${skipped ? ` (${skipped} skipped.)` : ""}`);
    ...
    qs("#btnRefresh", container).addEventListener("click", () => {
